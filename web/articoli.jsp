<%-- 
    Document   : articoli
    Created on : 14-giu-2019, 18.55.05
    Author     : Roberto97
--%>
            ...
            <div class="main-panel">
                ...
                <div class="content">
                    <div class="container-fluid">
                        ...
                        <div class="row">
                            <c:forEach var="articolo" items="${blogdao.getAllBlogs()}">
                                <c:set var="commenti" value="${commblogdao.getAllCommOfBlog(articolo.id).size()}" />
                                <div class="col-md-4">
                                    <div class="card card-profile">
                                        ...
                                        <div class="card-body">
                                            <h6 class="card-category text-gray"><i class="fas fa-stream mr-2"></i>${articolo.categoria}</h6>
                                            <h6 class="card-category text-gray">${articolo.creatore}</h6>
                                            <h4 class="card-title">${articolo.nome}</h4>
                                            <p class="card-description textOverflow">
                                                ${articolo.descrizione}
                                            </p>
                                            <p class="text-muted">
                                                <i class="far fa-calendar-alt"></i> ${articolo.data.toLocalDateTime().format(DateTimeFormatter.ofPattern("dd-MM-yyyy"))}
                                            </p>
                                            <div class="row">
                                                ...
                                                <div class="col-md-6 text-center">
                                                    <i class="far fa-eye"></i> ${articolo.views}
                                                </div>
                                            </div>
                                            <c:if test="${articolo.pubblicato eq false}">
                                                <div>
                                                    <h3>Non Pubblicato</h3>
                                                </div>
                                            </c:if>
                                            <a href="articolo.jsp?id=${articolo.id}" class="btn btn-outline-success">Modifica</a>
                                            <a href="/../Bortoleto/articolo/${articolo.id}/${articolo.nome.replace(' ', '-')}" class="btn btn-outline-warning" target="_blank" rel="noopener">Visualizza</a>
                                        </div>
                                    </div>
                                </div>
                            </c:forEach>
